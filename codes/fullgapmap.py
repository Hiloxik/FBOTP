...
T = 1
M = 100
J11_list = np.linspace(-(10*np.pi/4)/(np.sqrt(2)),(10*np.pi/4)/(np.sqrt(2)),M+1)
J12_list = np.linspace((10*np.pi/4)/(np.sqrt(2)),-(10*np.pi/4)/(np.sqrt(2)),M+1)
...
def Floquet1(J11, J12, ky):
    ...
    for i in range(1,2*N,2):
        hopping_xy1[i, i] = 1
    h1 = (2/T)*(np.kron(hopping_x1, s0)+np.kron(hopping_xy1, J2*s1))
...
def main():
    ...
    for i in range(M):
        ...
        for j in range(M):
            ...
            UF = Floquet(J11,J12)
            eval1,evec1 = np.linalg.eig(UF)
            eval2,evec2 = np.linalg.eig(UF)
            Eval = []
    ...
    np.savetxt('D:\科研\Topomat\Floquet insulator\FBOTP\\files\\'+'gap'+str(np.round(J2*(np.sqrt(2))/np.pi,2))+str(np.round(Gap,1))+'.txt', np.c_[gap],
    fmt='%.18e',delimiter='\t')
