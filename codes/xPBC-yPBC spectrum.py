...
def Floquet(J11,kx,ky):
    #Hamiltonians
    H1 = (2/T)*(J11*np.kron(s1,s0)-J12*np.kron(s2,s2))+(0*2/T)*(np.sin(ny*ky)*np.kron(s3,s1)+np.cos(ny*ky)*np.kron(s3,s2)-0.4*np.cos(ny*ky)*np.kron(s0,s0))+(0.0001)*(np.kron(s3,s0)+0.5*np.kron(s3,s3))
...
def main():
    for kx in kx_array:
        ...
        for ky in ky_array:
            ...
            E3.append(Eval[2])
            E4.append(Eval[3])
        plt.plot(ky_array,E1,color='red')
        plt.plot(ky_array,E2,color='red')
        plt.plot(ky_array,E3,color='red')
        plt.plot(ky_array,E4,color='red')
