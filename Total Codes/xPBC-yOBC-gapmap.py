...
T = 1
M = 50
J12_list = np.linspace((4*np.pi/4)/(np.sqrt(2)),0,M+1)
J11_list = np.linspace(0,(4*np.pi/4)/(np.sqrt(2)),M+1)
...
def Floquet(J11, J12, ky):
    ...
    for i in range(0,2*N-1,2):
        hopping_y1[i,i+1] = J11
        hopping_y1[i+1,i] = J11
    ...
    h2 = (2/T)*(np.kron(s3, hopping_y2,)+np.kron(J12*(np.cos(n*ky)*s1+np.sin(n*ky)*s2), np.eye(2*N)))
