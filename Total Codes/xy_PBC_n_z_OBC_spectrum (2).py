...
def Floquet(kx, ky):
    ...
    for i in range(1,2*N-1,2):
        hopping_z1[i,i+1] = 0
        hopping_z1[i+1,i] = 0
    H1 = (3/T)*np.kron(s0,np.kron(s0, hopping_z1))
    ...
    for i in range(1,2*N-1,2):
        hopping_z2[i,i+1] = J2
        hopping_z2[i+1,i] = J2
    H2 = (3/T)*np.kron(s0,np.kron(s0, hopping_z2))
    ...
    for i in range(1,2*N,2):
        hopping_z3[i,i] = -1
    H3 = (3/T)*np.kron(-(Jx*np.sin(ky)+Jy*np.sin(kx))*s4+(m+Jx*np.cos(ky)+Jy*np.cos(kx))*np.kron(s1,s0), hopping_z3)
    ...
    for i in range(len(kx_array)):
        ...
        for j in range(len(ky_array)):
            ...
            for l in range(len(Eval)):
                E[l][i,j] = Eval[l]
    for w in range(0,2):
        ax.plot_surface(X, Y, E[w], cmap='winter')
    for w in range(2,4*N-2):
        ax.plot_surface(X, Y, E[w], cmap='plasma')
    for w in range(4*N-2,4*N):
        ax.plot_surface(X, Y, E[w], cmap='winter')
    for w in range(4*N,4*N+2):
        ax.plot_surface(X, Y, E[w], cmap='autumn')
    for w in range(4*N+2,8*N-2):
        ax.plot_surface(X, Y, E[w], cmap='viridis')
    for w in range(8*N-2,8*N):
        ax.plot_surface(X, Y, E[w], cmap='autumn')
