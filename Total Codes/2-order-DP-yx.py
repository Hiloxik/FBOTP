...
J21 = (np.pi/4)/(np.sqrt(2))
J22 = (np.pi/4)/(np.sqrt(2))
d = 4
nx = 1
ny = 1
Nx = 5
    ...
    for theta in theta_array:
        ...
        for m in range(lt):
            ...
            for r in range(lx):
                Uy = []
                for i in range(2*ly):
                    Uy.append(U(-np.pi+r*((2*np.pi)/(Nx)),-np.pi+i*((2*np.pi)/(Ny)),t,theta))
                Uy.append(Uy[0])
                Uy_array.append(Uy)
            ...
            for i in range(ly):
                ...
                for e in range(lx):
                    ...
                    Wx = np.dot(QQx,Wx)
                print("det=",np.round(np.linalg.det(Wx),7),np.round(100*(((i+1+m*ly)/(ly*lt))),2),"%")
        ...
        z2 = 0*tt+1
        z3 = 0*tt+0
        plt.scatter(t_array,na1_array)
        plt.scatter(t_array,na2_array)
