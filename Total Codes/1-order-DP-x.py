...
def main():
    ...
    nu1_array = np.zeros((lt+1,ly+1))
    nu2_array = np.zeros((lt+1,ly+1))
    nu3_array = np.zeros((lt+1,ly+1))
    nu4_array = np.zeros((lt+1,ly+1))
    for k in range(lt):
        t = t_array[k]
        for j in range(ly):
            ky = ky_array[j]
            ...
            for i in range(lx):
                dk = i*((2*np.pi)/(N))
                UU0 = U(kx+dk,ky,t)
                UU1 = np.conjugate(U(kx+dk+(2*np.pi)/(N),ky,t)).T
    ...
    ax.plot_surface(X, Y, nu1_array, linewidth=0.1)  
