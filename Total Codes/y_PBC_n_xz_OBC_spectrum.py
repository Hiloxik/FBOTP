...
def Floquet(ky):
    ...
    for i in range(0,2*N-1,2):
        hopping_x[i,i] = m+np.cos(ky)
        hopping_x[i+1,i+1] = -m-np.cos(ky)
        hopping_x[i,i+1] = -1j*np.sin(ky)
        hopping_x[i+1,i] = 1j*np.sin(ky)
        
    H3 = (3/T)*phi*np.kron(hopping_x, hopping_z3)
...
def main():
    ...
    for ky in K_array:
        ...
        # edgewave3 = evec[:,(eval>0/T-0.025)&((eval<0/T+0.025))]
        ...
        #     p0 = np.reshape(NumToPlot0,(2*N,2*N)).T
        #     ax.plot_surface(X, Z, p0, cmap='plasma')
        #     ax.contour(X, Z, p0, 1000, zdir = 'z', offset = 0.06, cmap = plt.get_cmap('rainbow'))
